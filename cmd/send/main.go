...
func run() error {
	...
	if err != nil || u.Host == "" || (u.Scheme != "https" && u.Scheme != "http") {
		return fmt.Errorf("set a valid --url or RUBBERAI_URL")
	}
	if u.Scheme == "http" && u.Hostname() != "localhost" && u.Hostname() != "127.0.0.1" && u.Hostname() != "::1" {
		return fmt.Errorf("remote endpoints require HTTPS")
	}
	...
	if *status {
		...
		if json.Unmarshal(raw, &envelope) != nil {
			return fmt.Errorf("expected JSON event or events envelope on stdin")
		}
		...
		if len(list) < 1 || len(list) > 100 {
			return fmt.Errorf("send 1–100 events")
		}
		for i := range list {
			e := &list[i]
			if e.ProjectID == "" {
			...
			if e.ID == "" {
				...
				e.ID = "evt_" + hex.EncodeToString(b)
			}
		...
		body, err = json.Marshal(map[string]any{"events": list})
		if err != nil {
			return err
		}
	}
	...
	if err != nil {
		return fmt.Errorf("delivery failed; use the local collector and retry with the same explicit event IDs")
	}
	defer res.Body.Close()
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return fmt.Errorf("endpoint returned HTTP %d", res.StatusCode)
	}
	_, err = io.Copy(os.Stdout, io.LimitReader(res.Body, 4096))
	return err
}
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "rubberai-send:", err)
		os.Exit(1)
	}
}
