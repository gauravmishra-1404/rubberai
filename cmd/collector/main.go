...
func main() {
	...
	srv := &http.Server{Addr: "127.0.0.1:4319", Handler: c.Handler(), ReadHeaderTimeout: 5 * time.Second, ReadTimeout: 10 * time.Second, WriteTimeout: 10 * time.Second, IdleTimeout: 30 * time.Second}
	go c.Run(ctx)
	go func() {
		<-ctx.Done()
